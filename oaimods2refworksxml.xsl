...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/">
    <xsl:text disable-output-escaping="yes">&lt;?xml-stylesheet type="text/xsl" href="refworks2html.xsl"?&gt;
</xsl:text>
    <refworks xmlns:refworks="www.refworks.com/xml/">
      ...
      <xsl:for-each select="/oai:OAI-PMH/oai:ListRecords/oai:record">
        ...
        <reference inMyList="undefined" owner="true" id="{$refid}" isShared="true" genre="{$genre}">
          ...
          <id><xsl:value-of select="$doraid"/></id>
